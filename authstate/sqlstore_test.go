...
func newTestStore(t *testing.T, now func() time.Time, namespace string) (*testStore, func()) {
	t.Helper()
	...
	store, err := authstate.NewSQLStore[string](db, stringCodec{}, authstate.SQLOptions{Dialect: authsqlite.Dialect, Namespace: namespace, Now: now})
	if err != nil {
		db.Close()
		t.Fatal(err)
	}
	...
	return &testStore{SQLStore: store, db: db}, func() { _ = db.Close() }
}

// testStore keeps the database beside the store, because an external test
// package cannot reach the one the store holds.
type testStore struct {
	*authstate.SQLStore[string]
	db *sql.DB
}
...
func TestEnsureSchemaIsIdempotentAndRejectsIncompatibleTable(t *testing.T) {
	...
	db.SetMaxOpenConns(1)
	defer db.Close()
	store, err := authstate.NewSQLStore[string](db, stringCodec{}, authstate.SQLOptions{Dialect: authsqlite.Dialect, Namespace: "test"})
	if err != nil {
		t.Fatal(err)
	}
	if err := store.EnsureSchema(context.Background()); err != nil {
...
func TestStoreRejectsInvalidInputsAndCancellation(t *testing.T) {
	...
	defer db.Close()
	if _, err := authstate.NewSQLStore[string](db, stringCodec{}, authstate.SQLOptions{}); !errors.Is(err, authstate.ErrInvalidOptions) {
...
func TestStorePersistsAcrossReopen(t *testing.T) {
	...
	now := time.UnixMilli(1_800_000_000_000)
	open := func() (*authstate.SQLStore[string], func()) {
		db, err := dbsqlite.Open(path)
		if err != nil {
			t.Fatal(err)
		}
		db.SetMaxOpenConns(1)
		store, err := authstate.NewSQLStore[string](db, stringCodec{}, authstate.SQLOptions{Dialect: authsqlite.Dialect, Namespace: "test", Now: func() time.Time { return now }})
		if err != nil {
			db.Close()
			t.Fatal(err)
		}
