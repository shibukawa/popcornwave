...
func newTestStore(t *testing.T, fake *fakeItems, options Options) (*Store[payload], context.Context) {
	t.Helper()
	...
	store, err := NewStore[payload](session.JSONCodec[payload]{}, options)
	if err != nil {
		t.Fatal(err)
	}
...
func sampleRecord(now time.Time) session.Record[payload] {
	return session.Record[payload]{
		Data:            payload{UserID: "u-1"},
		CreatedAt:       now,
		AuthenticatedAt: now,
		LastSeenAt:      now,
...
func TestPutAndGetRoundTrip(t *testing.T) {
	...
	if got.Data.UserID != want.Data.UserID {
		t.Fatalf("payload = %q, want %q", got.Data.UserID, want.Data.UserID)
	}
