...
func TestRateLimitConfigValidation(t *testing.T) {
	cases := map[string]func(*RateLimitConfig){
		"per_address zero has no off position": func(c *RateLimitConfig) { c.PerAddress = 0 },
		"per_address negative":                 func(c *RateLimitConfig) { c.PerAddress = -1 },
		"per_address above the ceiling":        func(c *RateLimitConfig) { c.Process = 1; c.PerAddress = 2 },
		"per_subject above the ceiling":        func(c *RateLimitConfig) { c.Process = 2; c.PerSubject = 3; c.PerAddress = 1 },
		"unknown backend":                      func(c *RateLimitConfig) { c.Backend = "postgres" },
		"window not positive":                  func(c *RateLimitConfig) { c.Window = 0 },
		"redis dsn without redis backend":      func(c *RateLimitConfig) { c.Redis.DSN = "redis://localhost:6379" },
		"redis backend without dsn":            func(c *RateLimitConfig) { c.Backend = RateLimitBackendRedis },
	}
