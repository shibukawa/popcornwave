---
id: decision:canonical-popcornwave-skill-source
type: decision
title: Canonical Popcorn Wave Skill Source
---
Author the bundled framework skill once at repository-root `skills/popcornwave-skill/`; api:cli-init embeds that same tree and copies it to one selected agent discovery path.

```yaml
problem:
  - the repository source and api:cli-init output appear at different paths, which can look like two maintained skill trees
decision:
  canonical: skills/popcornwave-skill/
  rename: none
  embed: the Go skills package embeds the canonical directory directly
  generated_name: popcornwave
...
reasoning:
  ...
  - popcornwave-skill is the repository package name while popcornwave is the installed skill name; the differing names do not create a second source
rules:
  ...
  - documentation calls generated trees copies and points updates to skills/popcornwave-skill/
...
rejected:
  root_only_output:
    shape: generate only skills/popcornwave/ in an application project
  ...
  cosmetic_rename:
    shape: rename skills/popcornwave-skill to skills/popcornwave only to match installed directory names
...
```yaml
acceptance:
  - the repository contains one authored Popcorn Wave SKILL.md tree at skills/popcornwave-skill
