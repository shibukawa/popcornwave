...
CGO_ENABLED=1 "$go_bin" test ./contrib/database/cgosqlite ./contrib/database/sqlite

echo '== standard Go without cgo (modernc facade) =='
CGO_ENABLED=0 "$go_bin" test ./contrib/database/cgosqlite ./contrib/database/sqlite

echo '== forced TinyGo logic under standard Go (native driver) =='
CGO_ENABLED=1 "$go_bin" test -tags force_tinygo_logic ./contrib/database/cgosqlite ./contrib/database/sqlite
...
if [ -n "${TINYGO_DOCKER_IMAGE:-}" ]; then
	...
	docker run --rm -v "$ROOT:/src" -w /src \
		...
		./contrib/database/cgosqlite ./contrib/database/sqlite
elif command -v "${TINYGO:-tinygo}" >/dev/null 2>&1; then
	echo '== TinyGo native driver =='
	"${TINYGO:-tinygo}" test ./contrib/database/cgosqlite ./contrib/database/sqlite
else
