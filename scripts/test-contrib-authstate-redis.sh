...
# Run the authstate adapter against live Redis and Valkey containers.
set -eu

ROOT=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
cd "$ROOT"

redis_name=petitweb-authstate-redis-$$
valkey_name=petitweb-authstate-valkey-$$
redis_port=${REDIS_TEST_PORT:-16379}
valkey_port=${VALKEY_TEST_PORT:-16380}
...
echo '== Redis authstate interoperability =='
