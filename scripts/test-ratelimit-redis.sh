...
# Run the rate limit counter against live Redis and Valkey containers.
set -eu

ROOT=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
cd "$ROOT"

redis_name=petitweb-ratelimit-redis-$$
valkey_name=petitweb-ratelimit-valkey-$$
redis_port=${REDIS_TEST_PORT:-16383}
valkey_port=${VALKEY_TEST_PORT:-16384}
...
echo '== Redis rate limit counter interoperability =='
...
PETITWEB_REDIS_ADDR="127.0.0.1:$redis_port" \
	go test ./ratelimitstore/redis -run TestLiveRedisOrValkey -count=1
docker stop "$redis_name" >/dev/null

echo '== Valkey rate limit counter interoperability =='
