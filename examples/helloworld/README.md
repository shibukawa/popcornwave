...
Leave `--code-only` off: this example enables Tailwind, and that flag writes the
generated Go without rebuilding `public/generated/app.css`. The binary still
compiles, so the symptom of narrowing the command is a page served with a stale
