...
The SQL body stays SQL — nothing is translated between engines. The only dialect-dependent output is the placeholder token, decided by `project.database` in `popcornwave.toml`: `$1, $2, …` for `postgres`, `?` for `mysql` and `sqlite`. You always write `{name}`. Everything else (`||`, `ON CONFLICT`, `RETURNING`) reaches the SQL verbatim; write for the engine you selected.
...
- Putting a `.pw.sql` in a directory not listed under `generate.queries` in `popcornwave.toml` — reported, not compiled.
