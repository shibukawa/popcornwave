---
title: 5. 自分で更新するページ
description: ディレクトリからルートを生やし、Popcorn Wave のページが描画されたあとに変わる3つの方法を知る。
sidebar:
  order: 5
---

4章を終えた `memoapp` は、ごく普通のサーバーレンダリングのアプリケーションです。
`net/http` と `html/template` と1週間の注意深さがあれば同じものが作れますし、ここまでの
どこにも、このフレームワークが専用のテンプレート言語を持っている理由は出てきていません。

この章がその説明です。ウェブサイトがルートを増やすやり方——ディレクトリを作る——で
ルートを1つ足し、そのルートが描画されたあとにできることを3つ見ます。遷移で変わった領域だけを
返すこと、レスポンスが始まったあとに描画を終えること、そして誰かが見ているあいだ更新し続けること。
文字列を描画して終わりでは、どれもできません。
...
ページは入力をただの引数として宣言し、フレームワークに解析させることもできます。
`func Load(id string, page *int) (string, int, error)` がよくある形で、生成はこの戻り値の
並びをコンポーネントの引数の並びと突き合わせて検証します。

その形にはリクエストが入っておらず、コンテキストも入っていません。URL が運ぶのは
スカラーなので、型付きの入口が受け取るのもスカラーです。このページは `auth.User` と
データベースのプールを必要とし、どちらもリクエストのコンテキストに乗ってきます。だから
1段上を取ります。`func Load(w, r)` は登録だけを生成し、レスポンスはこちらに残します。

段はシグネチャで決まります。どちらの形にも当てはまらない `Load` は、いま持っている形と
取りうる2つの形を挙げて生成が止まります。
...
このレイアウトのチェインを覚えておいてください。次の節はこれの話です。
