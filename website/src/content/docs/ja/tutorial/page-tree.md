...
**読み込むだけ**のページは `Load` を書きません。テンプレートに external を宣言し、
結果を束縛し、実装をファイルの隣に置きます。
...
そのローダーは先頭に `context.Context` を宣言すればリクエストのものを受け取れます。
だから `auth.User` やデータベースのプールが要ることは、ハンドラに手を伸ばす理由には
なりません。どちらもそのコンテキストに乗ってきます。

このページがそれでも手を伸ばすのは、読み込むだけではないからです。**レスポンス全体を
決めています。** `func Load(w, r)` は登録だけを生成し、その後ろを全部こちらに残します。
ストリーミング・ダウンロード・条件付きステータスが必要とするのがそれです。

その形でも不在でもない `Load` は生成が止まり、取るべき形と、代わりに external を
バインドせよということを名指します。
