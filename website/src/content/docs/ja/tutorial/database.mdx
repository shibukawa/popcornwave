...
1章で断ったデータベースを、ここで入れます。

```sh
pw add database
```

ウィザードがエンジンを聞くので **SQLite** を選んでください。ファイル1つで動き、
別に起動しておくサーバーが要りません。確認画面を承認すると、`config.dev.toml` の
...
func createMemo(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		// 変更: 2章はここで memos.list() をそのまま渡していた。
...
// 追加: listMemos は流れてくる行を、テンプレートが描画するスライスにする。
