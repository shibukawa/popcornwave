...
あります。[http://127.0.0.1:18081](http://127.0.0.1:18081) のコンソールを開いて
**data** を選んでください。
...
predicate は[クエリー](/ja/guides/storage/queries/)にあります。

## 3. 呼ぶものが無いまま実行する

保存すると `pw dev` が再生成します。プロジェクトに Go の関数が2つ増えて、呼び出し側は
...
ペインがソースを写して見せているのではありません。アプリケーションが呼ぶのと同じ
...
もう1件入れてもかまいません。本物のテーブルの本物の行です。**data** ペインにも出ますし、
5節でそれを見つけることになります。
...
![id と body の列、保存された2行を表示する pw dev コンソールの memos テーブル](../../../../assets/screenshots/tutorial-database.png)
...
（[スロークエリー診断](/ja/productivity/query-diagnostics/)）。
...
- [クエリー](/ja/guides/storage/queries/) — 条件付き SQL、predicate、トランザクション。
- [リレーショナルデータベース](/ja/guides/storage/rdb/) — エンジン、プールの上限、リードレプリカ。
