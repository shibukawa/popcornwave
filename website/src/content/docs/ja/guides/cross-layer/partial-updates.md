...
これは響きより強い保証です。青の道は、誰かが保守しているフォールバックではありません。
リンクと GET フォームと戻るボタンが、自分だけでやることです。JavaScript を切っても
検索ページは検索できるし、絞り込みは絞り込めるし、アドレスバーには人に送れる URL が
残っています。そのための2つ目の実装があるからではなく、1つ目から存在しないからです。

緑の道も同じ基準で測られます。ランタイムが横取りする操作はすべて、ブラウザが自力で
辿り着いたはずの行き先に辿り着かなければなりません。両者が食い違うページは、ランタイムを
読み込まなかったページより悪いからです。以下で「ランタイムは横取りしない」と書いてある
箇所は、どれもこの規則を守っています。
...
`update()` が受け取るのは**クエリ全体**です。名指さなかったパラメータは落ちます。
GET フォームの送信がクエリをマージせず置き換えるのと同じで、残したいものは
`location.search` から読んで渡し直してください。
...
送信ボタンは、自分が引き起こす送信について発言権を持ちます。`formmethod`、
`formaction`、`formtarget` は、ランタイムがその送信を自分のものと判断するより先に
ボタンから読まれます。だから GET フォームの中の `<button formmethod="post">` は
POST でページを離れますし、押されたボタン自身の `name` と `value` は、他のどの送信
とも同じようにクエリに載ります。

いま開いている URL とフラグメントだけが違うリンクは、丸ごと放っておかれます。要素は
ブラウザが持っていて、どこへ送るかも知っています。往復しても同じページに着くだけです。
...
ドキュメントを解析し直す代わりに領域を差し替えると、既定ですべてが保たれます。それが
いつも望みどおりとは限りません。絞り込みは、読んでいた場所に読み手を残すべきです。
別のページへのリンクは、そうではありません。

だからランタイムは、どちらが起きたかで分けます。リンクと GET フォームは新しい場所に
着くところなので、先頭から —— あるいは名指したフラグメントから —— 始まり、差分が
フォーカスを持っていた要素を奪ったならページの `<main>` にフォーカスを移します。
`update()` は引数だけが違う同じページなので、ビューポートは動きません。

戻ると進むは、そのエントリが記録した位置を、領域が着地したあとで復元します。ブラウザ
自身の復元は切ってあります。ページをその高さにする内容が届く前に走るので、まだ存在
しない場所へスクロールしてしまうからです。

差し替えられる領域の中では、フォーカスを持っていたコントロールがフォーカスとキャレット
の両方を保ちます。効くのはキャレットのほうです。値はもともと運ばれていました。入力する
たびに更新する検索ボックスを使い物にならなくしていたのは、一打鍵ごとにカーソルが文末へ
飛ぶことでした。

入力メソッドに対しては、更新が待ちます。未確定の変換中に差分が着弾すると、変換していた
コントロールごと差し替えられ、綴じかけの文字が確定するか消えるかします。日本語や中国語の
入力では、検索ボックスでごく普通に起きることです。応答は変換が終わるまで保留され、待って
いる間に追い越された応答は、他のどの応答とも同じように破棄されます。

ナビゲーションか再描画が開いている間、ドキュメントのルート要素は `data-tb-updating` を
持ちます。これに CSS を当てれば、進捗表示はそれで終わりです。

```css
[data-tb-updating] .results { opacity: 0.6; }
```

自分で付けることはありません。リクエストが片付いた瞬間に消えます。
