...
すべての fallback をそのまま受け取ります。JavaScript を切ったブラウザにとっては、それは
まれで、承知のうえで劣化した経路です。しかし Googlebot や OGP スパイダー、`curl` にとっては
**それが唯一の経路**であり、結果として fallback がインデックスされる本文になり、シェア
カードの説明文になり、ターミナルに流れてくる中身になります。
