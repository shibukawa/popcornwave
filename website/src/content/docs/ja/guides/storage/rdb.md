---
title: リレーショナルデータベース
description: フレームワークが所有する接続の集合——エンジン、DSN、プールの上限、そしてステートメントの宛先になるリーダー・ライターのグループ。
sidebar:
  order: 0
---
...
| キー | 既定値 | 意味 |
| --- | --- | --- |
| `enabled` | `false` | フレームワークが所有するプールを開く |
...
行が[シードデータ](/ja/productivity/seed-data/)です。フレームワークが所有するテーブル——とりわけ
