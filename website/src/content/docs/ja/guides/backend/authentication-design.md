---
title: 認証の設計
description: 4つのモードが実際に分けているもの、要件ごとの選び方、そしてログインが終わったあとにセッションが持つ信頼度の層。
sidebar:
  order: 0
---
...
**引き受けるもの。** トークンは期限が来るまで有効なので、アクセスを早めに終わらせたいなら失効の戦略を自分で選ぶことになります。`revocation.mode` に緩い既定値は無く、決めるまで起動しません。[トークンの失効](/ja/guides/backend/token-revocation/)を参照してください。
