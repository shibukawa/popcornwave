...
`auth.mode` には 3 つの値がありますが、OIDC、パスキー、その併用という単純な
ログイン方式の選択ではありません。たとえば `oidc_passkey` では、普段のログインには
パスキーを使い、OIDC プロバイダが登場するのは復旧時だけです。

モードを選ぶときは、普段のログイン画面だけでなく、アカウントを誰が作り、誰が
復旧できるのかを基準にします。
...
![ブラウザ・Popcorn Wave・IdP の3者でリダイレクトとコード交換が進む通常ログインのシーケンス図](../../../../../assets/diagrams/login-sequence.svg)
