---
title: WebSocket
description: 双方向のプロトコルを入力と出力の 2 つの構造体として宣言し、接続の両向きのエンコードを生成に任せる。
sidebar:
  order: 3
---
...
プロキシの先でも生き延びますし、失敗は普通の problem レスポンスで返せます。まず
ストリームを検討してください。
...
フィールドは常に全部が線に乗ります。生成されるエンコーダに `omitempty` は無いので、
クライアントは判別用フィールドを読み、その種類が使わないフィールドを無視します。
