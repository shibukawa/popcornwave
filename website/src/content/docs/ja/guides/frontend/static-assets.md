...
プロジェクトのルートには `public/` があります。ここに置いたものは配信され、しかも
バイナリの外ではなく中に入ります。デプロイするファイルはひとつ、そして昨日のリリースの
スタイルシートが残る余地もありません。
...
スキャフォールドしたプロジェクトはもう配信できる状態にあります。

ただし、埋め込まれるのはそのディレクトリではありません。[`pw build`](/ja/pw/project/build/) が
`public/` を走査して `dist/public` を作り、バイナリが運ぶのは**そちら**です。何も変換しない
プロジェクトなら中身は完全な複製なので、この区別は必要になるまで何のコストもありません。
そして必要になった瞬間に効いてきます——ファイル名を変える変換に、結果の置き場所と、
参照を追随させる仕組みの両方が要るからです。
...
ビルドは変換の**後**に、圧縮対象のファイルへ `.zstd` の兄弟を書きます。圧縮されるのは実際に
出荷されるバイト列だ、というわけです。配信時の CPU コストはゼロになります。符号化済みの
バイト列がもう存在しているのですから。
...
| エンコーディング | `Accept-Encoding` が q 値 0 超で許し、*かつ*サイドカーがあるとき `zstd` |
...
レンダリングした HTML レスポンスを圧縮するのは別のスイッチで、トレードオフも別です。
