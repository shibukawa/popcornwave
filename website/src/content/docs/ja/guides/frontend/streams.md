...
`NewStream` が転送形式を決め、ステータスとヘッダを確定します。`Send` は値を 1 つ
書いてすぐに送信し、`Close` がレスポンスを完了させます。

ストリームは必ず閉じてください。JSON 配列形式では `Close` が閉じ括弧を書くため、
呼び出さないとクライアントには不正な JSON が届きます。値全体がすでに揃っている場合は、
通常のレスポンスを使ってください。ストリーミングにしても待ち時間は短くならず、切断や
途中まで送信したレスポンスの扱いだけが増えます。
