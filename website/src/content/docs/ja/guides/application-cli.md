...
`pw` は開発ツールです。`pw build` が生成するバイナリにも独自のコマンドラインがあり、
それは設定を駆動しているのと同じ宣言から生成されます。
...
どちらの形式も、フレームワークとアプリケーションの**登録済みすべての prefix** を
`default` 値付き、`help` をコメントにして出力します。バイナリは自分がインポートした
ものが登録した内容を報告するので、スキャフォールドは常に実際にリンクされている
パッケージと一致します。設定を登録する依存を追加すれば、次回の実行時にそれが現れます。

いずれの形式も出力後に終了し、サーバーは起動しません。

## 独自のサブコマンド

サブコマンドは構造体 1 つと登録呼び出し 1 つで作れます。`pw generate` が呼び出し箇所を
読んでパース処理を書き出すので、フラグの配線を保守する必要はありません。
...
サブコマンドはパース済みの設定を共有するので、`pw.Config[T]` はサーバーが使うのと同じ値
—— DSN を含めて —— をすでに返します。設定を二重に管理する必要はありません。
