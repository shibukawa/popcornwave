...
:::note[ブラウザの機能]
`<dialog>`、popover 系の属性、`<details>` はウェブプラットフォームの機能であり、
Popcorn Wave 独自のものではありません。フレームワークが関係するのは、同じ
マークアップを `.pw.html` コンポーネント内で扱うための、末尾のテンプレート規則
4 つだけです。
:::

以前は JavaScript ライブラリが必要だったモーダル、メニュー、ツールチップ、
アコーディオンの多くを、いまのブラウザは標準の要素と属性で実現できます。そのため、
サーバーで描画するコンポーネントでも、まずマークアップとして組み立てられます。
