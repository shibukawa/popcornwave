---
title: プロジェクト構成と設計原則
description: pw が作る開発環境、feature 単位のパッケージ、生成されたデータ境界を持つ net/http ハンドラという3つのスケールでプロジェクトを見る。
sidebar:
  order: 1
---

Popcorn Wave のプロジェクト構成は、フォルダツリーだけでは説明しきれません。その外側では
`pw` がアプリケーションをビルドして開発環境ごと動かし、内側では1つのハンドラが
リクエストからレスポンスまでを所有しています。フォルダ構造は、その間にあります。

3つのスケールで判断は変わりません。Go と Web で既に通じるインターフェースは隠さず、
その周囲で繰り返される仕事を、プロジェクトごとに組み直さなくてよい形で提供します。

## 開発環境を抱える `pw`

開発中のアプリケーションバイナリは、単独では動きません。`pw dev` がソースを監視し、
