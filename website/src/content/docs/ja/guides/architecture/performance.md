...
| ミドルウェア全体 | 2.7 µs |
| └ CSRF チェック | 2.2 µs |
| 50 行を返す `SELECT` 1 回 | 37 µs |
| JSON のエンコードと書き出し | 30 µs |
| HTML の描画と書き出し | 95 µs |
| リクエスト全体（Popcorn Wave） | 203 µs |
| リクエスト全体（`net/http` の比較実装） | 221 µs |
...
1 リクエストあたり約 0.5 µs です。一方、セッションレコード全体をレスポンスで送り、
ブラウザが次のリクエストで送り返すため、レコードが大きくなるほど通信量が増えます。
...
CSRF チェックはミドルウェアの中では目立つ処理ですが、上の計測でも 2.2 µs です。
