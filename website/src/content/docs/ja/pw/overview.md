...
`pw` は開発ツールです。プロジェクトのスキャフォールド、テンプレートと SQL の Go への
コンパイル、開発ループの実行、データベースの管理、リリースビルドを担当します。
...
`pw init` 以外のコマンドはプロジェクトの中で実行します。`pw` は作業ディレクトリから
上へたどって `popcornwave.toml` を探すので、どのサブディレクトリからでも動きます。
見つからない場合は `popcornwave.toml not found` で失敗します。
...
ビルドしたバイナリにも独自のコマンドラインがあります。設定オプション、スキャフォールド
出力、そして自分で定義したサブコマンドです。そちらは
