---
title: pw doctor
description: 指定した環境で何が動くのか、その裏に何がいるのか、どこが間違っているのかを、アプリケーションを起動せずに報告する。
sidebar:
  order: 7
---
...
[診断リファレンス](/appendix/diagnostics/)（英語）で引けます。このページはコマンドが
評価するのと同じカタログから生成されるので、エントリのないチェックは存在でき
ません。

秘密情報は**場所**で報告され、値では報告されません。指摘が名指しするのはキーと
ファイルで、値はレポートのどのセクションでもマスクされたままです。`--env=all` を
