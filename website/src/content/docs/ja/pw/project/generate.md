...
`pw generate` は `dist/routes.json` も書きます。アプリケーションが提供する全パターンが
1 つのファイルに入ります。Go で登録されたルート——それぞれが登録されたファイルと行付き
——と、ページツリーが生むルート。[ページツリー](/ja/tutorial/page-tree/)とハンドラ登録が別々の場所に持っている
URL 空間の、両方の半分です。

実行時には誰も読みません。ツールが解析をやり直さずに済むために存在します。
[`pw doctor`](/ja/pw/project/doctor/) が衝突するパターンを検査し、エディタ拡張が一覧に
します。`dist/manifest.json` と同じくビルド生成物で、スキャフォールドされた `.gitignore`
に含まれ、実行のたびに作り直されます。これより前に作られたプロジェクトには、無視行を
足す必要があります。

パターンがコンパイル時定数でない登録は読み取れないので、握り潰さずに未解決として記録
します。表を読むツールは、裏付けの取れないきれいなルート一覧を出すかわりに、その旨を
述べます。
