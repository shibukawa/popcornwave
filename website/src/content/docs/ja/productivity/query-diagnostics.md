...
失敗した文にもレコードが出ます。`outcome=error` が付き、`bind_values` が on のときは
ドライバー自身のメッセージがそれを起こした SQL の隣に並びます。この組み合わせだけで
原因が分かることも少なくありません。`bind_values` が off のときは、メッセージの代わりに
`error.type`(テキストではなく分類)が入ります。ドライバーのメッセージは
`Key (email)=(…) already exists` のように違反した値をそのまま埋め込むことが多く、その値は
`bind_values` がログから締め出すための行データそのものだからです。
