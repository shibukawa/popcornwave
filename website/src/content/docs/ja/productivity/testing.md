...
truncate してから挿入するのは既定であって、唯一の選択肢ではありません。テーブルごとに
`_operation` で選べます。

| 操作 | 効果 |
| --- | --- |
| `clear-insert`（既定） | テーブルを空にしてから、並んだ行を挿入する |
| `insert` | すでにある行を残したまま、並んだ行を挿入する |
| `upsert` | 並んだ行を挿入する。主キーが存在すれば更新する |
...
キーは単数形の `_operation` です。`_operations` と書いても何も選ばれません。認識されない
トップレベルのキーはテーブル名として読まれるため、パーサは「行のリストが来るはずの
ところにマッピングがある」と報告します。操作の話は一言も出てきません。行のタグのキーが
`_tag` であることも、同じ落とし穴です。
