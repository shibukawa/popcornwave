...
同じようにスコープされます。一般規則は[スタイリング](/ja/guides/styling/)にあります。
...
[フラグメントと島](/ja/interaction/fragments/)にあります。

### インラインスクリプトの中括弧はテンプレート構文

`{` はテンプレートのボディならどこでも式を開きます。`<script>` の中も同じです。
これは実行時の驚きではなく生成エラーです。
...
スクリプトは `public/` の下のファイルに置いて参照してください。`script-src`
ポリシーが望むのも結局これです。
...
どうしてもインラインにするなら、信頼済みの内容として渡します。

```html
<script type="module">{RawJavaScript("customElements.define('copy-button', CopyButton);")}</script>
```

`RawJavaScript` はサニタイザではありません —— エスケープ規則は
[テンプレート](/ja/guides/templates/)にあります —— ので、固定文字列に限ってください。

例外の位置は 1 つだけあり、見た目より狭いものです。コンポーネントの
`<head>` **寄与ブロック**は内容がそのまま扱われるため、中の中括弧は安全です。
一方でドキュメントシェル自身の `<head>` は普通のテンプレートマークアップで、他と
同じように解析されます。[ナビゲーション](/ja/interaction/navigation/)の Speculation
Rules が素の JSON リテラルではなく `RawJavaScript` を通っているのはそのためです。
...
[フラグメントのレシピ](/ja/interaction/fragments/)が取っている形です。
...
- 遷移なしに変化する領域は、変化したことを伝えるべきです。`aria-live="polite"` を
  付けてください。[フラグメントと島](/ja/interaction/fragments/)の swap レシピは
  これに依存しています。
