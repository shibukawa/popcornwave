...
閉じタグはありません。束縛は書かれた位置で走り、名前はそこから**囲んでいるブロックの終わり**
まで読めます。囲んでいる要素、制御構造の本体、あるいはコンポーネント本体です。そのブロックを
出れば名前は解決しません。
...
- **同じブロックで同じ名前を2回束縛すること。** 外側の名前を隠すのは `for` や `await` と
  同じく黙って通ります。同一ブロックの2つ目は再宣言で、しかも閉じタグが無いので1つ目の
  有効範囲を読み取る手がかりがありません。
