...
at [http://127.0.0.1:18081](http://127.0.0.1:18081) and take **data**. The
...
Save the file and `pw dev` regenerates. There are two new Go functions in the
...
That is not the pane echoing your source back. It called the same generated
...
Insert a second memo if you like. They are real rows in the real table — the
**data** pane shows them, and section 5 finds them there.
...
![the pw dev data pane showing the memos table with its id and body columns and two persisted rows](../../../assets/screenshots/tutorial-database.png)
