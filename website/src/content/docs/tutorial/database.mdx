...
Chapter 1 declined the database. Install it now:

```sh
pw add database
```

The wizard asks for an engine; choose **SQLite**, which is one file and no
server to start beside the application. Accept the review screen and you get
...
func createMemo(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		// changed: chapter 2 passed memos.list() straight in here.
...
// new: listMemos turns the streamed rows into the slice the template renders.
