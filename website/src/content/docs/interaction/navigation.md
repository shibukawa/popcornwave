...
the click. They belong to the document shell, which is where the one template
detail bites: `{` opens an expression in template markup, and a rules block is
mostly braces. Hand the JSON over as trusted content:
...
<html lang="en"><head>
  <meta charset="utf-8">
  <title>My App</title>
  <script type="speculationrules">{RawJavaScript('{"prerender":[{"where":{"href_matches":"/tasks/*"},"eagerness":"moderate"}]}')}</script>
</head>
<body><slot /></body></html>
}
```

A fixed string is exactly the case `RawJavaScript` exists for. The alternative
avoids the inline script altogether — point at the rules with a response header,
which is also what a strict `script-src` policy prefers:
