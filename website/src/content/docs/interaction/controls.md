...
contents the same way. See [Styling](/guides/styling/) for the general rule.
...
[Fragments and islands](/interaction/fragments/).

### A brace inside an inline script is template syntax

`{` opens an expression everywhere in the body of a template, including inside
`<script>`. This is a generation error, not a runtime surprise:
...
Put the script in a file under `public/` and reference it — which is what a
`script-src` policy wants anyway:

```html
<script type="module" src="/public/copy-button.js"></script>
```

If it truly has to be inline, hand it over as trusted content:

```html
<script type="module">{RawJavaScript("customElements.define('copy-button', CopyButton);")}</script>
```

`RawJavaScript` is not a sanitiser — see the escaping rules in
[Templates](/guides/templates/) — so keep it to fixed strings.

One position escapes the rule, and it is narrower than it first looks. A
component's `<head>` **contribution block** is taken verbatim, so braces inside
it are safe. The document shell's own `<head>` is ordinary template markup and
is parsed like everything else — which is why the speculation rules in
[Navigation](/interaction/navigation/) reach it through `RawJavaScript` rather
than as a plain JSON literal.
...
[fragment recipes](/interaction/fragments/) use.
...
- A region that changes without a navigation should say so. Mark it
  ...
  [Fragments and islands](/interaction/fragments/) depend on it.
