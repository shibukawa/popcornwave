...
// NewConnection describes one pool for the pane.
func NewConnection(label, group, driver string, readOnly bool, db *sql.DB) Connection {
	return Connection{
...
func NewSingle(db *sql.DB, driver, environment string) *Server {
...
func (c *Connection) MigrationState(ctx context.Context) (Migration, error) {
	...
	if err := row.Scan(&state.Version, &state.Applied); err != nil {
