...
// Exporter sends completed spans to a telemetry backend.
type Exporter interface {
	ExportSpans(context.Context, []SpanData) error
}

// SimpleProcessor synchronously exports each completed span. It is useful for tests.
...
func (p *SimpleProcessor) OnEnd(span SpanData) {
	if p == nil || p.exporter == nil {
		return
	}
	if err := p.exporter.ExportSpans(context.Background(), []SpanData{span}); err != nil {
		p.mu.Lock()
...
// BatchConfig bounds the production processor's memory and flush latency.
...
// BatchProcessor exports bounded batches on a background goroutine.
// A full queue drops new spans without blocking request handling.
...
func NewBatchProcessor(exporter Exporter, config BatchConfig) *BatchProcessor {
	...
	p := &BatchProcessor{exporter: exporter, config: config, queue: make(chan SpanData, config.QueueSize), done: make(chan struct{})}
	go p.run()
	return p
}

func (p *BatchProcessor) OnEnd(span SpanData) {
	if p == nil {
		return
	}
	p.mu.RLock()
...
func (p *BatchProcessor) run() {
	...
	defer ticker.Stop()
	batch := make([]SpanData, 0, p.config.MaxExportSize)
	flush := func() {
