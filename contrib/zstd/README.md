...
`contrib/zstd` is an encoder-only RFC 8878 package for TinyGo and Go web
...
- TinyGo builds use Petitweb's bounded pure-Go encoder
- `go build -tags force_tinygo_logic` forces TinyGo-compatible contrib logic
  on host Go, including the Petitweb encoder in this package
...
a 128 KiB window, lower-memory mode, and no frame checksum. The Petitweb
...
The Petitweb backend additionally omits Huffman literals, general FSE tables,
