...
static int petitweb_tls_connect(int fd, const char *hostname, uintptr_t *out) {
	*out = 0;
	petitweb_tls *state = (petitweb_tls *)calloc(1, sizeof(petitweb_tls));
...
static int petitweb_tls_read(uintptr_t raw, void *buf, int len, int *out) {
	petitweb_tls *state = (petitweb_tls *)raw;
...
static int petitweb_tls_write(uintptr_t raw, const void *buf, int len, int *out) {
	petitweb_tls *state = (petitweb_tls *)raw;
	if (state == NULL || state->ssl == NULL) return -300;
...
static void petitweb_tls_close(uintptr_t raw) {
	petitweb_tls *state = (petitweb_tls *)raw;
