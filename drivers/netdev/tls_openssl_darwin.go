...
func sysTLSConnect(fd int, hostname string) (uintptr, error) {
	...
	tlsConnectMu.Lock()
	rc := int(C.petitweb_tls_connect(C.int(fd), (*C.char)(unsafe.Pointer(&name[0])), &state))
	tlsConnectMu.Unlock()
	if rc != 0 {
...
func sysTLSSend(state uintptr, buf []byte) (int, error) {
	...
	var n C.int
	rc := int(C.petitweb_tls_write(C.uintptr_t(state), unsafe.Pointer(&buf[0]), C.int(len(buf)), &n))
	if rc != 0 {
...
func sysTLSRecv(state uintptr, buf []byte) (int, error) {
	...
	var n C.int
	rc := int(C.petitweb_tls_read(C.uintptr_t(state), unsafe.Pointer(&buf[0]), C.int(len(buf)), &n))
	if rc != 0 {
		return -1, tlsError(rc)
	}
	return int(n), nil
...
func sysTLSClose(state uintptr) {
	if state != 0 {
		C.petitweb_tls_close(C.uintptr_t(state))
	}
}
